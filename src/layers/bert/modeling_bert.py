...
class BertImgModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # embedding是[4, 200, 768]
        # shape应该是 token_num * token_dim
        self.embeddings_1 = nn.Parameter(torch.rand(4, 768, dtype=torch.float16))
        self.embeddings_2 = nn.Parameter(torch.rand(4, 768, dtype=torch.float16))
        self.embeddings_3 = nn.Parameter(torch.rand(3, 768, dtype=torch.float16))
        self.embeddings_4 = nn.Parameter(torch.rand(7, 768, dtype=torch.float16))
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        range_len = input_ids.shape[0]
        
        embedding_output_mod = torch.Tensor([]).type(torch.float16).to(embedding_output.device)
        for i in range(range_len):
            ...
            embedding_tmp = embedding_output[i]
            embeddings_tmp = torch.cat((embedding_tmp[:-60], self.embeddings_1, embedding_tmp[-56: index_dot1 + 1], self.embeddings_2, embedding_tmp[index_dot1+5: index_dot2+1], self.embeddings_3, embedding_tmp[index_dot2+4: index_dot3+1], self.embeddings_4, embedding_tmp[index_dot3+8:]), 0) 
            embeddings_tmp.unsqueeze_(0)
            embedding_output_mod = torch.cat((embedding_output_mod, embeddings_tmp), 0)

        assert embedding_output_mod.shape == embedding_output.shape

        # embedding_output_mod = torch.Tensor([]).type(torch.float16).to(embedding_output.device)
        # for i in range(range_len):
        #     input_list = input_ids[i].tolist()
        #     index_dot1 = input_list.index(1012, -42)
        #     index_dot2 = input_list.index(1012, index_dot1 + 1)

        #     embedding_tmp = embedding_output[i]
        #     embeddings_tmp = torch.cat((embedding_tmp[:-42], self.embeddings_1, embedding_tmp[-38: index_dot1 + 1], self.embeddings_2, embedding_tmp[index_dot1+5: index_dot2+1], self.embeddings_3, embedding_tmp[index_dot2+4:]), 0) 
        #     embeddings_tmp.unsqueeze_(0)
        #     embedding_output_mod = torch.cat((embedding_output_mod, embeddings_tmp), 0)
        ...
        if img_feats is not None:
            ...
            embedding_output = torch.cat((embedding_output_mod, img_embedding_output), 1)
...
class BertCaptioningLoss(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        loss = self.kl(log_prb, one_hot).sum(1) # 每个样本只有一个元素为 1，其他位置都为 0，因此在计算 KL 散度时，只需要考虑这个元素对应的 log-softmax 值和真实标签的值即可。
...
class BertForImageCaptioning(BertPreTrainedModel):
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask,
            ...
            is_training=True, encoder_history_states=None, score=None, difficulty=None):
        ...
        if is_training:
            ...
            class_logits = self.cls(sequence_output_masked) # [142, 30522]
            masked_ids = masked_ids[masked_ids != -1]   # remove padded target [142]
    ...
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None,
            ...
            decoding_constraint_flag=None, bad_ending_ids=None, score=None, difficulty=None
            ):
