...
class VisionLanguageTSVDataset(object):
    def __init__(self, args, yaml_file, tokenizer, tensorizer=None, is_train=True, on_memory=False):
        ...
        self.template = json.load(open('./datasets/MTL_AQA_5_en/template.json'))
        ...
        self.visual_tsv = self.get_tsv_file(self.visual_file) # TSVFile(tsv_file='datasets/MSVD/frame_tsv/train_32frames.img.tsv')

        self.label_file = self.cfg.get('label', None)
        self.label_tsv = self.get_tsv_file(self.label_file) # TSVFile(tsv_file='datasets/MSVD/train.label.tsv')

        self.cap_file = self.cfg.get('caption', None)
        self.cap_tsv = self.get_tsv_file(self.cap_file) # TSVFile(tsv_file='datasets/MSVD/train.caption.tsv')

        # socre annotation file
        self.score_dict = pkl.load(open(self.cfg.get('score', None), 'rb'))
    ...
    def get_frames_from_tsv(self, binary_frms):
        ...
        frames = []
        _C, _H, _W = 3, 224, 224
        if self.decoder_num_frames > len(binary_frms): # 不进来
        ...
        def sampling(start,end,n): # 0, 31, 32
            if n == 1:
        ...
        def select_numbers(num_list):
            selected_nums = []
            for i in range(0, len(num_list), 3):
                group = num_list[i:i+3]
                selected_num = random.choice(group)
                selected_nums.append(selected_num)
            return selected_nums
        
        num_list = list(range(0, 96))
        sample_list = select_numbers(num_list)
        is_unique = len(sample_list) == len(set(sample_list))
        assert is_unique, "抽取的32帧必须是独特的"
    ...
    def get_visual_data(self, idx, start=None, end=None):
        ...
        # return a video-frame tensor 进到下面
    ...
    def get_gt_score(self, img_key):
        key = img_key.split('/')[-1].split('_')
        key = (int(key[0]), int(key[1]))
        return self.score_dict[key]['final_score']
    
    def get_difficulty(self, img_key):
        key = img_key.split('/')[-1].split('_')
        key = (int(key[0]), int(key[1]))
        return self.score_dict[key]['difficulty']
    ...
    def __getitem__(self, idx):
        ...
        """
        这里进行了修改！！！新加入的，要获得对应的模板
        """
        slash_pos = img_key.rfind('/')
        slash_vid = img_key[slash_pos+1:]
        template = self.template[slash_vid]

        caption_sample, tag, start, end = self.get_caption_and_timeinfo_wrapper(img_idx, cap_idx)  
        # get image or video frames (start end 都是 none)
        # frames: (T, C, H, W),  is_video: binary tag  /  raw_frames [32, 3, 360, 640]
        ...
        if self.args.add_od_labels==True:
            example = self.tensorizer.tensorize_example_e2e(caption, preproc_frames, text_b=template, text_meta=caption_sample)
        else:
            example = self.tensorizer.tensorize_example_e2e(caption, preproc_frames, text_meta=caption_sample)
        example += (self.get_gt_score(img_key), self.get_difficulty(img_key))
