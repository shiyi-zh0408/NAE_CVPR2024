...
class CaptionTensorizer(object):
    ...
    def get_attn_masks(self, seq_a_len, seq_len):
        # image features
        img_len = self.max_img_seq_len # 784

        max_len = self.max_seq_len + self.max_img_seq_len # 984
        # C: caption, L: label, R: image region
        c_start, c_end = 0, seq_a_len # 0, 104
        l_start, l_end = self.max_seq_a_len, seq_len # 135, 191
        r_start, r_end = self.max_seq_len, self.max_seq_len + img_len # 200, 984
    ...
    def mask_text_inputs(self, tokens, seq_a_len, seq_len, text_meta=None):
        if self.is_train:
            ...
            # mlm_targets = None
            # 测试时，mask为全1，即全部被mask，mlm_target是全None
            # masked_pos = torch.ones(self.max_seq_len, dtype=torch.int)
            """
            这里进行了修改！！！
            """
            masked_pos = torch.zeros(self.max_seq_len, dtype=torch.int)
            masked_idx = range(0, self.max_seq_a_len)
            masked_pos[masked_idx] = 1
            mlm_targets = None
