...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    # 121935，总共需要经历的iter数（iter指batchsize个样本训练一次）
    max_iter = args.max_iter
    # 121935（gradient参数表示多少iter更新一次参数）
    max_global_step = args.max_global_step
    # 121935/15=8129（一个epoch需要多少次iter）
    global_iters_per_epoch = args.global_iters_per_epoch
    ...
    # 只有一个循环（没体现遍历epoch），但是train_dataloader里面做了epoch的循环
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        inputs = {
            ...
            'score': batch[6].float(), 'difficulty': batch[7].float()
        }

        """ 第一个6是batch_size,140和最大长度相关
        input_ids = torch.Size([6, 140]): caption的token id
        attention_mask = torch.Size([6, 924, 924]): attention mask, word tokens + video tokens组成的mask矩阵,这里都还是固定的,video部分全是1
        token_type_ids = torch.Size([6, 140]): 这个是啥？[应该是BERT训练next sentence任务用的]
        img_feats = torch.Size([6, 32, 3, 224, 224]) num_vid_token=16 * 7 * 7=784 num_text_tokens=50
        masked_pos = torch.Size([6, 140]): mask掉的token的位置,用1表示mask掉,0表示不被mask掉
        masked_ids = torch.Size([6, 50]): mask掉的token的id
        """
        if iteration == 1:
        ...
        if args.mixed_precision_method == "fairscale":
            ...
            # 前向
            # [Training]: Compute MLM loss
            # it's a classification task
            # return a tuple of (masked_loss, class_logits, ...)
            # class_logits = (mask_num, vocab_size)
            outputs = model(**inputs)
        loss, logits = outputs[:2]
        loss_aqa_ce = outputs[-5]
        loss_aqa_mse = outputs[-6]
        weight_vid = outputs[-1]
        weight_prompt = outputs[-2]
        # if loss_aqa_mse < 10:
        #     loss_aqa = loss_aqa_mse + loss_aqa_ce
        # elif loss_aqa_mse < 1000:
        #     loss_aqa = 0.01 * loss_aqa_mse + loss_aqa_ce
        # else:
        #     loss_aqa = 0.001 * loss_aqa_mse + loss_aqa_ce
        loss_aqa = loss_aqa_mse + loss_aqa_ce
        loss = loss + 0.1 * loss_aqa

        if args.learn_mask_enabled:
            loss_sparsity = outputs[-7]
            # 总损失
        ...
        if args.learn_mask_enabled:
            loss_dict = {'loss': loss, 'loss_sparsity': loss_sparsity.item(), 'loss_aqa': loss_aqa.item(), 'loss_aqa_mse': loss_aqa_mse.item(), 'loss_aqa_ce': loss_aqa_ce.item(), 'weight_vid': weight_vid, 'weight_prompt': weight_prompt, 'acc': batch_acc}
        ...
        if backward_now:
            ...
            if (args.save_steps > 0 and global_step % args.save_steps == 0) or global_step == max_global_step or global_step == 1:
                ...
                if get_world_size() > 1:
                    dist.barrier()
                # 先把save_model关了，改成20个epoch存一次，不然太占空间
                ...
                if args.evaluate_during_training:
                    ...
                    evaluate_file = evaluate(args, val_dataloader, model, tokenizer, checkpoint_dir, epoch)
                    if get_world_size() > 1:
                        dist.barrier()
                    if is_main_process():
...
def evaluate(args, val_dataloader, model, tokenizer, output_dir, epoch):
    # 这里是初始化
    predict_file = get_predict_file(output_dir, args,
            val_dataloader.dataset.yaml_file)
    # 得到predict_file
    test(args, val_dataloader, model, tokenizer, predict_file, epoch)
    ...
    if is_main_process():
        ...
        logger.info(f'predict_file: {predict_file}')
        logger.info(f'caption_file: {caption_file}')
        logger.info(f'evaluate_file: {evaluate_file}')
...
def eval_aqa(pred_scores_dict: dict, score_json_path: str):
    gt_dict = json.load(open(score_json_path, 'r'))
    pred_score_list = []
    gt_score_list = []
    for key, value in pred_scores_dict.items():
        v_id = key.split('/')[-1]
        gt_score = gt_dict[v_id]
        pred_score = value
        pred_score_list.append(pred_score)
        gt_score_list.append(gt_score)

    pred_score_list = np.array(pred_score_list)
    gt_score_list = np.array(gt_score_list)
    rho, _ = stats.spearmanr(pred_score_list, gt_score_list)
    RL2 = np.power((pred_score_list - gt_score_list) / (gt_score_list.max() - gt_score_list.min()), 2).sum() / \
          gt_score_list.shape[0]
    return rho, RL2

def test(args, test_dataloader, model, tokenizer, predict_file, epoch):
    # TODO，check一下怎么做到不输入text_token
        ...
        with torch.no_grad():
            for step, (img_keys, batch, meta_data) in tqdm(enumerate(test_dataloader)):
                ...
                inputs = {'is_decode': True,
                    ...
                    'token_type_ids': batch[2], 'img_feats': batch[3],
                    'masked_pos': batch[4],
                    'score': batch[-2].float(), 'difficulty': batch[-1].float(),
                    'do_sample': False,
                ...
                for img_key, pred_score in zip(img_keys, pred_scores):
                    pred_scores_dict[img_key] = pred_score
                for img_key, pred_cls in zip(img_keys, pred_clss):
                    pred_cls_dict[img_key] = pred_cls
    ...
    # rho, rl2 = eval_aqa(all_data, '/videocap/matrix_aqa_eval/score.json')
    # rl2 = 100 * rl2
    # logger.info(f"aqa evaluation result rho: {rho:.3f}  rl2: {rl2:.3f} epoch: {epoch}")
    
    rank = get_rank()

    with open(f'{args.pred_scores_path}/epoch_{epoch}_rank_{rank}.json', 'w') as f:
        json.dump(pred_scores_dict, f)
    with open(f'{args.pred_cls_path}/epoch_{epoch}_rank_{rank}.json', 'w') as f:
        json.dump(pred_cls_dict, f)
...
def check_arguments(args):
    ...
    # TODO这个地方对 attn_mask_type 进行了修改，不知道是什么
...
def get_custom_args(base_config):
    ...
    # score_emb_num [Modified by Shiyi Zhang]
    parser.add_argument('--score_emb_num', type=int, default=100)
    ...
    # 加入分数预测的路径
    output_dir = parser.parse_args().output_dir.split('/')[-1]
    parser.add_argument('--pred_scores_path', type=str, default=f'/videocap/predict_scores/{output_dir}')
    parser.add_argument('--pred_cls_path', type=str, default=f'/videocap/predict_cls/{output_dir}')
...
def main(args):
    ...
    # 单卡为 device(type='cuda')
    args.device = torch.device(args.device)
    # Setup CUDA, GPU & distributed training
    dist_init(args)
    check_arguments(args)
    mkdir(args.output_dir)
    mkdir(args.pred_scores_path)
    mkdir(args.pred_cls_path)
    ...
    # 总共有8GPU，现在是第0个
    # device: cuda, n_gpu: 8, rank: 0, 16-bits training: deepspeed, fp16, -1
    logger.info(
        "device: {}, n_gpu: {}, rank: {}, "
    ...
    logger.info(f"Pytorch version is: {torch.__version__}") # 1.8.0a0+52ea372
    logger.info(f"Cuda version is: {torch.version.cuda}") # 11.2
    logger.info(f"cuDNN version is : {torch.backends.cudnn.version()}" ) # 8100
    ...
    vl_transformer = VideoTransformer(args, config, swin_model, bert_model, tokenizer)


    # # 加载预训练权重
    # pretrained_dict = torch.load('/videocap/AQA_output_CLIPnew4/checkpoint-60-9900/model.bin')
    # # 获取当前模型的参数和权重
    # model_dict = vl_transformer.state_dict()
    # # 删除文本解码器中的所有权重
    # excluded_layers = []
    # for name in pretrained_dict.keys():
    #     if 'trans_encoder' in name:
    ...
    # # 加载更新后的权重
    # vl_transformer.load_state_dict(model_dict)


    vl_transformer.freeze_backbone(freeze=args.freeze_backbone) # 训练时候 False

    total_params = sum(p.numel() for p in vl_transformer.parameters())
    logger.info(f'Model total parameters: {total_params}')

    # 加载模型参数（若有），训练时候 False
    if args.do_eval:
    ...
    if args.do_train:
        ...
        # TODO：这个地方需要check
        # 正常来说len(train_dataloader)代表了有多少个batch！而不是总共有多少个数据，len(dataloader.dataset)这是有多少数据
        args.max_iter = len(train_dataloader) # 2647500，总共需要经历的iter数（iter指batchsize个样本训练一次）
        dataset_len = len(train_dataloader.dataset) # 5295，总共有多少数据（有多少个caption）
        args.max_global_step =  args.max_iter// args.gradient_accumulation_steps # 总共更新2647500次参数（gradient参数表示多少iter更新一次参数）
        args.global_iters_per_epoch = args.max_global_step // args.num_train_epochs # 2647500/500=5295（一个epoch需要多少次iter）
